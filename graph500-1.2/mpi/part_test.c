...
//////
  ...
  for (i=0; i < g.nglobalverts; ++i) { if (parts[i] == rank) { ++perm_local_size; } }
  ...
  for (bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx) {
    ...
    if (rank == 0) fprintf(stderr, " %d", bfs_root_idx);
  ...
  if (rank == 0) {
    if (!validation_passed) {
      ...
      */
      ...
      fprintf(stdout, "NNZ_WEIGHT:                     %f\n", (float)NNZ_WEIGHT);
      fprintf(stdout, "GAMMA:                          %f\n", (float)F_GAMMA);
      fprintf(stdout, "DEGREE CUTOFF:                  %d\n", F_CUTOFF);
    }
  }
