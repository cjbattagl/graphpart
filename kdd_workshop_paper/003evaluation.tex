...
We ran \ourmethod on a representative selection of these graphs, and outline the results in~\RefTable{tab:rw} and in the following section on quality.
...
In~\RefTable{tab:rmatpmstrong} we show strong scaling times for two RMAT scales using ParMETIS. Increasing the number of processes causes a reduction in partition quality, with a mild sublinear speedup. We did not 
...
\subsection{Quality}
In \RefTable{tab:rw} we show some properties of our real test-graphs, as well as the performance of our streaming partitioner on them, for $k=2$ and $k=8$. 
