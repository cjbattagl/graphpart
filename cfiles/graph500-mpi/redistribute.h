...
void RF_FUNCNAME(const int* parts, RF_EXTRA_PARAMS) { \
  ...
  for (i = 0; i < bufsize; ++i) { \
    ...
    if (v0 == v1) continue; \
    RF_CALL_ON_EDGES(v0, v1, lg_nglobalverts, MAKE_CSR_REDISTRIBUTE_FUNC_COUNT_EDGE_BY_OWNER) \
  } \
  ...
  for (i = 0; i < bufsize; ++i) { \
    ...
    int64_t v1 = get_v1_from_edge(&buf[i]); \
    if (v0 == v1) continue; \
    RF_CALL_ON_EDGES(v0, v1, lg_nglobalverts, MAKE_CSR_REDISTRIBUTE_FUNC_STORE_EDGE_TO_BUFFER) \
  } \
